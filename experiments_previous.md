...
- **전략**:
  ...
  2.  **그룹 정의**:
      - [자금 흐름]: 보험료납입, 제지급, 대출
      - [계약 상태]: 계약해지, 계약정보, 명의변경
      - [서류/증명]: 증명서 안내장, 각 도메인
  3.  **Hybrid 유지**: 키워드 규칙 유지.

---

## 실험 12: Group-First Thinking 결과 (10의 연장)

- **날짜**: 2025-12-21
- **결과 (150개 샘플링)**:
  - **정확도**: 61.33% (92/150) - **실험 9 대비 하락**
- **13개 상위 도메인 기준 분석**:
  - **정확도**: 63.76% (21개 분류보다 +2.01%p 높음)
- **핵심 발견**:
  1.  **그룹핑의 역설**: "납입중지"가 `자금 흐름` 그룹으로 쏠리면서 `계약정보`(GT) 오분류 심화. 그룹핑이 오히려 세부 도메인 간의 경계를 흐림.
  2.  **Ground Truth의 한계**: LLM은 "돈을 안 내는 것"을 [자금 흐름]으로 보지만, GT는 [계약 유지 관리]로 봄. 이 간극은 프롬프트로 메우기 힘듦.
  3.  **구조적 개편 필요**: 21개 도메인 체계 자체가 LLM의 RAG 문서 검색 목적에 비효율적임.

---

## 실험 13: LLM 친화적 도메인 재구성 (13개 Super Domain) + 정교한 Rule

- **날짜**: 2025-12-21 (진행 예정)
- **가설**: 인간 업무 기준의 21개 도메인을 **LLM이 이해하기 쉽고 문서 검색에 최적화된 13개 도메인**으로 통합/재편하고, **코드 레벨의 정교한 규칙(Regex)**을 결합하면 정확도가 대폭 향상될 것이다.
- **전략**:
  1.  **도메인 매핑**: 21개 -> 13개로 통합 (예: 계약정보+고객정보+명의변경 -> [계약 및 고객 관리])
  2.  **프롬프트 전면 수정**: 13개 도메인 체계로 정의 다시 작성 및 Few-shot 예시 제거 (단순화).
  3.  **Keyword Routing 강화 (코드 레벨)**: `청약철회` -> `계약 해지 및 취소`, `납입중지` -> `보험료 납입`, `증권 재발행` -> `계약 및 고객 관리` 등 13개 도메인에 맞춰 정규식(Regex) 기반의 강력한 규칙 적용.
  4.  **평가 기준 변경**: 21개 GT를 13개로 매핑하여 평가.
- **예상 도메인 (13개)**:
  1. [계약 및 고객 관리] (통합)
  2. [보험료 납입] (명확화)
  3. [보험금 수령/지급] (명확화)
